    ...
    public static void run(FileWriter fileWriter, Workbook workbook) throws IOException {
        ...
        sheet.forEach(row -> {
            ...
            if (row.getRowNum() > 0) { //Ignore headers
                try {
                    fileWriter.write(Templates.club(dbUniqueIDStart.get(), cell(row.getCell(0)), cell(row.getCell(1)), cell(row.getCell(2)), cell(row.getCell(3)), cell(row.getCell(4)), cell(row.getCell(5)), cell(row.getCell(6)), cell(row.getCell(7)), cell(row.getCell(8)), cell(row.getCell(9))));
                } catch (IOException e) {
